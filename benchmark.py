...
class CUDABenchmarker:
    ...
    def benchmark_kernel(self, kernel_key, kernel_info):
        ...
        if "ensemble_config" in kernel_info and kernel_info["ensemble_config"]:
            ...
            configs_to_test.append(("Ensemble DNN", ens["block_x"], ens["block_y"], ens.get("block_z", 1)))
        
        if "random_forest_config" in kernel_info and kernel_info["random_forest_config"]:
            rf = kernel_info["random_forest_config"]
            configs_to_test.append(("Random Forest", rf["block_x"], rf["block_y"], rf.get("block_z", 1)))
        
        # Fallback to optimal_config if no specific model configs
        if not configs_to_test:
            suggested = kernel_info["optimal_config"]
            configs_to_test.append(("Suggested", suggested["block_x"], suggested["block_y"], suggested.get("block_z", 1)))
        ...
        for model_name, model_data in model_results.items():
            ...
            print(f"\n{model_name}:")
            print(f"  Speedup vs best: {speedup_vs_best:.2f}×  vs avg: {speedup_vs_avg:.2f}×")
    ...
    def print_summary(self, results):
        ...
        for model_name in sorted(model_names):
            beats_best = sum(1 for r in results if r.get(f"{model_name}_beats_best", False))
            beats_avg = sum(1 for r in results if r.get(f"{model_name}_beats_avg", False))
            
            speedups_best = [r[f"{model_name}_speedup_vs_best"] for r in results if f"{model_name}_speedup_vs_best" in r]
            speedups_avg = [r[f"{model_name}_speedup_vs_avg"] for r in results if f"{model_name}_speedup_vs_avg" in r]
        ...
        for model_name, stats in model_stats.items():
            print(f"\n{model_name}:")
            print(f"  Beats best random: {stats['beats_best']}/{total} ({100*stats['beats_best']/total:.0f}%)")
            print(f"  Beats avg random:  {stats['beats_avg']}/{total} ({100*stats['beats_avg']/total:.0f}%)")
